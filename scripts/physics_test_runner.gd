...
func _validate_and_quit() -> void:
	...
	var carrier_delta := abs(snapshot["carrier"]["theta"] - baseline["carrier"]["theta"])
	var sun_delta := abs(snapshot["sun"]["theta"] - baseline["sun"]["theta"])
	var dial_delta := abs(snapshot["dial"]["theta"] - baseline["dial"]["theta"])
	var flywheel_delta := abs(snapshot["flywheel"]["theta"] - baseline["flywheel"]["theta"])
	var bell_ok := abs(snapshot["bell_omega"]) > 0.03 or snapshot["strike_energy"] > 0.02
	var planets_ok := abs(snapshot["planet_a"]["omega"]) > 0.20 and abs(snapshot["planet_b"]["omega"]) > 0.20
	var transmission_ok := snapshot["momentum_score"] > 2.1 and snapshot["ring_to_dial_power"] > 0.003
	var escapement_ok := abs(snapshot["balance"]["theta"] - baseline["balance"]["theta"]) > 0.08 and abs(snapshot["last_escape_impulse"]) > 0.01
	var belt_ok := modality["belt"]["energy"] > 0.002 and modality["belt"]["flywheel_speed"] > 0.05
	var cam_ok := modality["cam_follower"]["height"] > 6.0 and modality["cam_follower"]["energy"] > 0.02
	var hammer_ok := modality["hammer_bell"]["energy"] > 0.01 and bell_ok
	var ratchet_ok := modality["ratchet"]["index"] >= 1 and modality["ratchet"]["energy"] >= 1.0
	var geneva_ok := modality["geneva"]["energy"] >= 1.0 and abs(snapshot["geneva"]["theta"] - baseline["geneva"]["theta"]) > 0.10
	var stable_ok := snapshot["max_abs_omega"] < 120.0 and snapshot["constraint_error"] < 8.0 and snapshot["sanitize_ok"]
