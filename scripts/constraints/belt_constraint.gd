...
var a: String = ""
var b: String = ""
var stiffness: float = 0.62
var min_tension: float = 0.18
var max_slip: float = 5.0
var ratio_bias: float = 0.08
...
func solve(rotors: Dictionary, dt: float) -> void:
	...
	if is_nan(last_slip) or is_inf(last_slip):
		...
		last_impulse = 0.0
		return
	var tension: float = clampf(1.0 - abs(last_slip) / max(max_slip, EPSILON), min_tension, 1.0)
