...
var wheel := ""
var balance := ""
var engage_angle := 0.32
var coupling_ratio := 0.42
var impulse_scale := 0.72
var tick_scale := 0.42
...
func solve(rotors: Dictionary, _dt: float) -> void:
	...
	if denom <= EPSILON:
		last_impulse = 0.0
		return
	var impulse := -impulse_scale * engage * c_dot / denom
	rw.omega += impulse / rw.inertia
	rb.omega += coupling_ratio * impulse / rb.inertia
	var directional_input := -rb.theta
	if abs(directional_input) <= EPSILON:
		directional_input = -rw.omega
	var tick_strength := tick_scale * engage * sign(directional_input)
